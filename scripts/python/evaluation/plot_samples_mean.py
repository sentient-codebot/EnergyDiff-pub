...
from energydiff.utils.plot import plot_sampled_data, plot_sampled_data_v2
...
def main():
    ...
    # denormalize
    target = data.denormalize_fn(target, *scaling_factor).squeeze(1).mean(dim=0, keepdim=True) # [b, l]
    for k, v in generated.items():
        generated[k] = data.denormalize_fn(v, *scaling_factor).squeeze(1).mean(dim=0, keepdim=True) # [b, l]
    ...
    fig = plot_sampled_data_v2(samples={'Real Data': target, 'GMM': generated['gmm'], 'EnergyDiff': generated['ddpm']},
                         save_filepath=f'results/figures/samples/debug/samples_{config.model.load_runid}.pdf',
