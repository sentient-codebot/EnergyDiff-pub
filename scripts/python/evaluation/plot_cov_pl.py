...
from energydiff.utils.plot import plot_acf_comparison, plot_cov_comparison
...
def main():
    ...
    # Log to wandb 
    # original_run.log({"cov": wandb.Image(fig, file_type='png')}) # pdf type not avail
    
    try:
        fig = plot_cov_comparison(
        dict_cov={
            'VAE': dict_cov['vae'],
            # 'VAE Calibrated': dict_cov['vae_calibrated'],
            'GAN': dict_cov['gan'],
            # 'GAN Calibrated': dict_cov['gan_calibrated'],
        },
        filename=f"cov_{config.time_id}_vae_gan.pdf"
    )
        # original_run.log({"cov+": wandb.Image(fig, file_type='png')})
    except Exception as e:
        print(f"Error: {e}")
        pass
