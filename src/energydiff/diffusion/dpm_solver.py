...
from .diffusion_1d import GaussianDiffusion1D, ModelMeanType, ModelVarianceType
...
def model_wrapper(
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            return noise - cfg_scale * expand_dims(sigma_t, dims=cond_grad.dim()) * cond_grad
        elif guidance_type == "classifier-free":
            if cfg_scale == 1. or unconditional_condition is None:
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',
        ...
        if method == 'adaptive':
            ...
            with torch.no_grad():
                ...
                for step in tqdm(range(order, steps + 1), desc='multistep_dpm_solver_update'):
            ...
            for i, order in tqdm(enumerate(orders), desc='singlestep_dpm_solver_update'):
...
class DPMSolverSampler(object):
    def __init__(self, model: GaussianDiffusion1D, **kwargs):
        ...
        self.register_buffer('alphas_cumprod', to_torch(model.alpha_cumprod), device=device)
